...
**Recalibrage du 08/09/2026, sur le guide de migration Opus 5 livré avec Claude Code.** Trois ajustements. La narration : le guide demande une phrase avant le premier appel d'outil et des points d'étape sur ce qui change la direction, et classe « don't narrate » parmi les consignes écrites contre des modèles bavards, qui font sous-narrer les modèles actuels — l'interdit de narration est donc devenu une règle positive avec son « quand ». Les auto-corrections : le modèle « narrates self-corrections more than prior models », et une simple question de suivi déclenche un ré-audit d'un travail correct — d'où « corriger sans cérémonie ». Le périmètre : la doc dit avoir ajouté à son propre snippet la clause « finish the whole task, only report completion when it's fully done », parce que la discipline de périmètre seule ne couvrait pas les « terminé » prématurés — la clause est rétablie, ainsi que « dire la vérité sur l'état réel », retirées toutes deux en 1.8.0 au motif que le prompt système de Claude Code les porte : la skill tourne aussi dans claude.ai et via l'API, où il n'y a pas ce prompt. Ce recalibrage n'a pas été mesuré sur Opus 5 : il suit la doc.
