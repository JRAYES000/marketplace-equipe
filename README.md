...
| `linkedin-mise-en-forme` | 1.1.0 | 22/09/2026 | Règles de forme obligatoires pour tout post LinkedIn de Julien, sur les deux comptes : accroche formulée en question qui interpelle ou inquiète, au moins huit passages en gras (tous sans accent — le gras Unicode de LinkedIn n'a pas de lettre accentuée), trois sections portant chacune un titre en gras précédé d'un emoji, 3 à 6 emojis en tête de ligne, phrases de 20 mots maximum, corps entre 1 300 et 1 900 caractères. `scripts/verif-post.mjs verif <brouillon>` mesure désormais douze critères (contre neuf) et sort en code 1 si l'un échoue ; `scripts/verif-post.mjs gras "<segment>"` fabrique le gras et refuse un segment accentué. Corrige un vrai bug trouvé le 22/09/2026 sur les deux posts de veille déjà publiés : le script ne reconnaissait qu'une des deux polices « gras » Unicode réellement en usage et comptait à tort « 0 gras trouvé » ; ajoute une banque de formulations interdites (reprise de `linkedin-carrousel`) et un garde-fou contre une accroche dégénérée. Logique extraite dans `scripts/lib.mjs`, 15 tests (`npm test`). Se combine avec `contenu-linkedin`, qui dit quoi écrire. |
