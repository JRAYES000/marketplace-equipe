...
**Pour vérifier que ça a marché :** tu dois voir les cinq skills dans tes capacités, chacune précédée du nom du paquet — `skills-equipe:phrase-magique`, `skills-equipe:fonce`, `skills-equipe:parallelisation-et-routage`, `skills-equipe:delegation-deepseek-openrouter`, `skills-equipe:ponytail`.

## 📦 Les skills de l'équipe

Une fois `skills-equipe` installé, les skills arrivent avec le paquet. Tu n'as rien à télécharger.

**Paquet publié : `skills-equipe` 1.10.0.** Si le numéro affiché dans *Réglages → Plugins* est inférieur à 1.10.0, tu n'as pas la dernière version.

**Aucune skill ne s'active toute seule.** Les cinq attendent que tu les appelles — en tapant leur nom, ou avec une des formulations données dans la colonne *Ce qu'elle fait*. Une fois lancée, une skill tient jusqu'à ce que tu dises `mode normal`. C'est volontaire : rien ne change ta façon de travailler sans que tu l'aies décidé. Si une skill se déclenche alors que tu ne l'as pas demandée, c'est un bug — ouvre une issue.
...
| `ponytail` | 1.0.1 | 15/08/2026 | Le mode « moins de code ». Avant d'écrire, Claude se demande dans l'ordre : est-ce que ça doit exister, est-ce que c'est déjà dans le projet, est-ce que le langage ou le navigateur le fait déjà tout seul, est-ce qu'une bibliothèque déjà installée suffit, est-ce que ça tient en une ligne. Il s'arrête au premier « oui ». Le vrai gain est le deuxième point : réutiliser ce qui existe déjà dans le dépôt au lieu de le réécrire à côté — c'est l'erreur la plus fréquente, et elle n'a pas disparu avec Opus 5. Il ne simplifie jamais la sécurité, la validation des données, la gestion d'erreur ni l'accessibilité, et si tu redemandes la version complète il la fait sans discuter. Dis « ponytail » ou tape `/skills-equipe:ponytail` ; « ponytail souple » pour qu'il construise ce que tu demandes en te signalant seulement l'option plus simple ; `mode normal` pour l'arrêter. |
...
> **`ponytail` a quitté cette liste.** Il est maintenant dans notre paquet, sous le nom `skills-equipe:ponytail` — une version allégée, en français, recalibrée pour Opus 5. **Si tu avais installé le magasin `DietrichGebert/ponytail`, désinstalle-le** : garder les deux, c'est le même module en double. Une différence à connaître : la version publique s'activait toute seule en permanence, la nôtre s'appelle quand tu en as besoin (« ponytail »). C'est volontaire — on ne voulait pas l'imposer à tout le monde sur tous les projets.
>
