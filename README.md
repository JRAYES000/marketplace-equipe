...
| `linkedin-mise-en-forme` | 1.1.1 | 22/09/2026 | Règles de forme obligatoires pour tout post LinkedIn de Julien, sur les deux comptes : accroche formulée en question qui interpelle ou inquiète, au moins huit passages en gras (tous sans accent — le gras Unicode de LinkedIn n'a pas de lettre accentuée), trois sections portant chacune un titre en gras précédé d'un emoji, 3 à 6 emojis en tête de ligne, phrases de 20 mots maximum, corps entre 1 300 et 1 900 caractères. `scripts/verif-post.mjs verif <brouillon>` mesure désormais douze critères (contre neuf) et sort en code 1 si l'un échoue ; `scripts/verif-post.mjs gras "<segment>"` fabrique le gras et refuse un segment accentué. Corrige deux vrais bugs trouvés le 22/09/2026 en rebranchant `linkedin-veille-virale` dessus : le script ne reconnaissait qu'une des deux polices « gras » Unicode réellement en usage (comptait à tort « 0 gras trouvé ») et coupait un titre contenant une apostrophe en deux segments illisibles. Ajoute une banque de formulations interdites (reprise de `linkedin-carrousel`) et un garde-fou contre une accroche dégénérée. Logique extraite dans `scripts/lib.mjs`, 17 tests (`npm test`). Se combine avec `contenu-linkedin` (une skill de projet, dans le dépôt `visibilite-ops` — pas une skill de ce paquet, donc pas toujours chargeable selon le dossier de travail), qui dit quoi écrire. |
| `linkedin-carrousel` | 1.3.3 | 22/09/2026 | « Fais-moi un carrousel » : compose un carrousel LinkedIn (PDF de 8 à 12 pages, 10 par défaut) pour Claude Agency ou Claude Partners à partir d'un sujet, avec ses propres garde-fous — nombre et longueur des diapos, règles d'écriture du texte du post (longueur, gras, emojis, accroche, chiffres sourcés, accents) — qui refusent tout contenu hors des règles plutôt que de le signaler seulement. Publication réelle (image de couverture ou par-diapo) désormais fonctionnelle pour Claude Partners ; Claude Agency reste en attente. Les échecs de publication (pas seulement les succès) sont désormais journalisés. L'image de couverture affiche désormais le logo même sur une diapo hook (bug corrigé le 18/09/2026). La diapo hook met désormais un mot/chiffre-clé du titre en couleur de marque (inspiré de 3 carrousels de référence envoyés par Julien) — l'accent s'affiche maintenant réellement sur les images "couverture" (régression corrigée le même jour, trouvée en production sur le premier post de veille). Corrige le 22/09/2026 un faux positif de `valider-orthographe.js` (mot d'un gras accentué par erreur détecté sur le texte non converti) trouvé en auditant les trois skills LinkedIn — sans impact connu sur cette skill (l'ordre de ses propres contrôles l'évitait déjà), corrigé par prudence dans les trois copies dupliquées. `node etat.js` donne un point de situation avant de commencer. |
| `linkedin-commentaires` | 1.2.6 | 22/09/2026 | « Les commentaires du jour » : trouve des posts récents (moins de 4h en priorité) sous des comptes ciblés pour Claude Agency ou Claude Partners, et rédige un commentaire sur mesure dans l'un des quatre genres prévus — jamais une expérience inventée sans confirmation réelle, jamais plus de 5 par jour ni deux fois la même personne. Publie désormais chaque compte sur son bon canal (Claude Agency et Claude Partners ne partagent plus la même connexion), et journalise aussi les échecs de publication, pas seulement les succès. Même correctif de `valider-orthographe.js` que `linkedin-carrousel` (22/09/2026), par prudence. `node etat.js` donne le quota du jour avant de commencer. |
| `linkedin-veille-virale` | 0.3.0 | 22/09/2026 | « Fais la veille du jour » : surveille des comptes LinkedIn suivis, calcule un vrai score d'engagement (réactions, commentaires, partages rapportés aux abonnés) et propose un post recyclé/inspiré pour Claude Agency ou Claude Partners — jamais deux fois le même jour, jamais plus de trois par semaine, vérifié contre un registre réel. Rebranché le 22/09/2026 sur les douze critères de `linkedin-mise-en-forme` (import réel, `lib/valider-mise-en-forme.js`) au lieu des 3 critères repris jusque-là de `linkedin-carrousel` — les deux posts déjà publiés (Bernard Marr, Andrew Ng) n'auraient obtenu que 6/12 sur le vrai barème. Corrige au passage un faux positif de `valider-orthographe.js` (un mot volontairement sans accent dans un titre/une accroche en gras était signalé comme une faute) et réécrit les deux rédactions d'exemple pour qu'elles respectent réellement les douze critères. La voie réelle actuelle (Buffer, geste manuel) reste hors de portée de ce garde-fou. `node etat.js` donne un point de situation avant de commencer. |
