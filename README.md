...
| `linkedin-carrousel` | 1.3.2 | 21/09/2026 | « Fais-moi un carrousel » : compose un carrousel LinkedIn (PDF de 8 à 12 pages, 10 par défaut) pour Claude Agency ou Claude Partners à partir d'un sujet, avec ses propres garde-fous — nombre et longueur des diapos, règles d'écriture du texte du post (longueur, gras, emojis, accroche, chiffres sourcés, accents) — qui refusent tout contenu hors des règles plutôt que de le signaler seulement. Publication réelle (image de couverture ou par-diapo) désormais fonctionnelle pour Claude Partners ; Claude Agency reste en attente. Les échecs de publication (pas seulement les succès) sont désormais journalisés. L'image de couverture affiche désormais le logo même sur une diapo hook (bug corrigé le 18/09/2026). La diapo hook met désormais un mot/chiffre-clé du titre en couleur de marque (inspiré de 3 carrousels de référence envoyés par Julien) — l'accent s'affiche maintenant réellement sur les images "couverture" (régression corrigée le même jour, trouvée en production sur le premier post de veille). `node etat.js` donne un point de situation avant de commencer. |
| `linkedin-commentaires` | 1.2.5 | 21/09/2026 | « Les commentaires du jour » : trouve des posts récents (moins de 4h en priorité) sous des comptes ciblés pour Claude Agency ou Claude Partners, et rédige un commentaire sur mesure dans l'un des quatre genres prévus — jamais une expérience inventée sans confirmation réelle, jamais plus de 5 par jour ni deux fois la même personne. Publie désormais chaque compte sur son bon canal (Claude Agency et Claude Partners ne partagent plus la même connexion), et journalise aussi les échecs de publication, pas seulement les succès. `node etat.js` donne le quota du jour avant de commencer. |
| `linkedin-veille-virale` | 0.2.1 | 21/09/2026 | « Fais la veille du jour » : surveille des comptes LinkedIn suivis, calcule un vrai score d'engagement (réactions, commentaires, partages rapportés aux abonnés) et propose un post recyclé/inspiré pour Claude Agency ou Claude Partners — jamais deux fois le même jour, jamais plus de trois par semaine, vérifié contre un registre réel. Réutilise désormais les règles hook/gras/emoji du carrousel (refus avant toute publication directe) ; la voie réelle actuelle (Buffer, geste manuel) reste hors de portée de ce garde-fou. `node etat.js` donne un point de situation avant de commencer. |
