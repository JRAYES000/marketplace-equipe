...
**Pour vérifier que ça a marché :** tu dois voir les quatre skills dans tes capacités, chacune précédée du nom du paquet — `skills-equipe:phrase-magique`, `skills-equipe:fonce`, `skills-equipe:parallelisation-et-routage`, `skills-equipe:delegation-deepseek-openrouter`.

## 📦 Les skills de l'équipe

Une fois `skills-equipe` installé, les skills arrivent avec le paquet. Tu n'as rien à télécharger.

**Paquet publié : `skills-equipe` 1.11.0.** Si le numéro affiché dans *Réglages → Plugins* est inférieur à 1.11.0, tu n'as pas la dernière version.

**Aucune skill ne s'active toute seule.** Les quatre attendent que tu les appelles — en tapant leur nom, ou avec une des formulations données dans la colonne *Ce qu'elle fait*. Une fois lancée, une skill tient jusqu'à ce que tu dises `mode normal`. C'est volontaire : rien ne change ta façon de travailler sans que tu l'aies décidé. Si une skill se déclenche alors que tu ne l'as pas demandée, c'est un bug — ouvre une issue.
...
| `delegation-deepseek-openrouter` | 1.4.0 | 23/08/2026 | Fait faire les grosses tâches (résumés, traductions, gros volumes) par une IA moins chère. Tu économises tes messages Claude. À réserver au vrai volume : sur une tâche moyenne, baisser l'effort de réflexion de Claude coûte moins cher que l'aller-retour. Nouveau : Claude choisit lui-même entre la petite IA, la grande, et lui-même, en se posant trois questions dans l'ordre. La grande coûte dix fois la petite pour presque rien de mieux, donc elle ne sert que sur un cas précis — beaucoup de texte à produire, sans enjeu, mais avec du raisonnement dedans. |
